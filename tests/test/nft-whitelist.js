...
let signs = [];
context('NFT Contract With Whitelisting', async () => {
    var swapContract;
    var app;
    var ethersProvider;
    var isSaleOpen, tokensLeft, indivMaxAmount;
    var currentTime;
    ...
    it('should deploy Fixed NFT Swap Contract with whitelist', mochaAsync(async () => {
        /* Create Contract */
    ...
    it('GET - Purchase ID', mochaAsync(async () => {     
        ...
        expect(Number(purchase.amountContributed).toFixed(2)).to.equal(Number(tradeValue).noExponents());
        expect(purchase.purchaser).to.equal(app.account.getAddress());
        expect(purchase.reverted).to.equal(false);
    }));


    it('GET - tokensLeft after Swaps', mochaAsync(async () => {        
        let tokens = await swapContract.tokensLeft({categoryId: 1});
